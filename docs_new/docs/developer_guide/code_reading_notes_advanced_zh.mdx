...
本篇是 [部署与开发精读](/docs/developer_guide/code_reading_notes_zh) 的进阶续篇，按实现级粒度覆盖九个子系统。路径相对仓库根；类名/函数名以当前代码为准。配套：[SRT 核心服务路径](/docs/developer_guide/code_reading_srt_core_zh)、[Mem Cache / Models / Layers / Sampling](/docs/developer_guide/code_reading_deep_dive_zh)、[服务扩展模块（API / Grammar / VLM）](/docs/developer_guide/code_reading_serving_extensions_zh)。
