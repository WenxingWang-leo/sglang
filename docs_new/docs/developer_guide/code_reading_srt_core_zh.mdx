...
本页是 [代码精读笔记（部署与开发）](/docs/developer_guide/code_reading_notes_zh) 的**深度续篇**，只覆盖 SRT 主 serving 路径。目标读者：要改调度 / IPC / 启动装配 / forward 边界的工程师。路径相对仓库根；线号以当前 `main` 附近代码为准，读时请对照源文件。
...
| 调度策略 / continuous batching | `schedule_policy.py` `PrefillAdder`、`get_next_batch_to_run` |
